...
void countingSort(int *size,int *k,int digitPos,int *iSet,int *count2,int *slot3)
{
    ...
    for(i=0;i<10;i++)
    {
        printf("\n %d . %.3d - COUNT; %d %.3d - SLOT",i,count2[i],i,slot3[i]);
    }
   
   printf("\nK Value = %d, NEw Size = %d, Found Index = %d; \nFirst Value = %d \n",*k,tempSize,index2,iSet[0]);
    int tempStore;
...
int main(void) {
   ...
   do
    {   
        ...
        printf("\n\nPosition %d n %d ; K value = %d; First Value = %d",(9-digitPosition),sizeI,kValue,inputSet[0]);
    ...
    printf("\nMSD 10 Radix: The value with rank %d is",origKValue);
    
    printf("\nMSD 10 Radix: The value with rank %d is %d",origKValue,finalResult);
